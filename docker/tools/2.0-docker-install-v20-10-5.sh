...
#docker install
#2020-07-27 Guanwf
#2021-03-18 Guanwf 调整版本docker-20.10.5
...
if [ ! -f "docker-20.10.5.tgz" ];then
  log "docker-20.10.5.tgz file not exists."
  exit 1;
fi
...
if [ ! -f "./docker-20.10.5.tgz" ];then
  ...
  tar -zxvf docker-20.10.5.tgz -C ../
  cp ../docker/* /usr/bin/
else
  log "docker-20.10.5.tgz file is exists."
fi
tar -zxvf docker-20.10.5.tgz -C ../
cp ../docker/* /usr/bin/
...
ExecStart=/usr/bin/dockerd --graph ${DOCKER_HOME}/docker/dockerimage
